<div x-data="{
    ...
    }" x-show="$wire.isOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black bg-opacity-95 z-50 flex flex-col" @carousel-open.window="$wire.openCarousel()"
    @carousel-close.window="$wire.close()" @keydown.left.window="$wire.isOpen && goPrevious()"
    @keydown.right.window="$wire.isOpen && goNext()" @keydown.escape.window="$wire.isOpen && $wire.close()"
    @keydown.space.window="$wire.isOpen && (($event.preventDefault()), goNext())" tabindex="0">
    ...
    <!-- Main Content Area -->
    <div class="flex-1 flex flex-col overflow-hidden" @touchstart="onTouchStart($event)"
        @touchmove="onTouchMove($event)" @touchend="onTouchEnd($event)"
        @click="$event.target === $event.currentTarget && $wire.close()" style="height: calc(100vh - 160px);">
        <!-- Reserve space for header (80px) and navigation (80px) -->
        ...
        <div class="flex-1 relative overflow-hidden">
            ...
            <div class="flex h-full items-center transition-transform duration-300 ease-out"
                ...
                <div class="flex-shrink-0 flex items-center justify-center px-4 py-2"
                    style="width: 100vw; height: 100%;" wire:key="carousel-card-{{ $item->id }}-{{ $index }}">
                    <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden max-h-full flex flex-col"
                        ...
                        <div class="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200">
                            ...
                            @if($item->organic)
                            <div
                                class="absolute top-4 left-4 bg-green-500 text-white px-3 py-1 rounded-full text-sm font-semibold shadow-lg">
                                Organic
                            </div>
                            @endif
                            ...
                            <div
                                class="absolute top-4 right-4 bg-black bg-opacity-80 text-white px-4 py-2 rounded-full backdrop-blur-sm">
    ...
    <div
        class="flex items-center justify-between p-4 bg-black bg-opacity-50 backdrop-blur-sm border-t border-white border-opacity-10">
        <!-- Previous Button -->
