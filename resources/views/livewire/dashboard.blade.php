                        ...
                        @if(isset($pluInsights['category_breakdown']) && $pluInsights['category_breakdown']->count() > 0)
                            <div>
                                ...
                                <p class="text-sm text-gray-600">
                                    ...
                                    <br>
                                    <span class="text-xs text-gray-500">{{ $topCategory->count }} of {{ $totalItems }} items</span>
                                </p>
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <div class="flex-1 bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full" style="width: {{ $pluInsights['organic_ratio'] ?? 0 }}%"></div>
                                </div>
                                <span class="ml-2 text-sm text-gray-600">{{ $pluInsights['organic_ratio'] ?? 0 }}%</span>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">
                                {{ $pluInsights['organic_items'] ?? 0 }} of {{ $pluInsights['total_items'] ?? 0 }} items are organic
                            </p>
                        </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div class="text-center">
                            <p class="text-2xl font-semibold text-blue-600">{{ $marketplaceInsights['total_views'] ?? 0 }}</p>
                            <p class="text-xs text-gray-500">Total Views</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-semibold text-green-600">{{ $marketplaceInsights['total_copies'] ?? 0 }}</p>
                            <p class="text-xs text-gray-500">Total Copies</p>
                        </div>
                    </div>

                    @if(isset($marketplaceInsights['top_performing']) && $marketplaceInsights['top_performing']->count() > 0)
                            ...
                            <div class="space-y-2">
                                @foreach($marketplaceInsights['top_performing'] as $list)
                                    <div class="flex justify-between items-center text-xs">
                                        <span class="text-gray-900 truncate">{{ $list->marketplace_title ?? $list->name }}</span>
